...
#define SDA* portOutputRegister(digitalPinToPort(7))                                    // pin   MEGA  UNO
#define open_line0 {*portOutputRegister(digitalPinToPort(8))=digitalPinToBitMask(8);}   // D8  = PH5   PB0
#define open_line1 {*portOutputRegister(digitalPinToPort(9))=digitalPinToBitMask(9);}   // D9  = PH6   PB1
#define open_line2 {*portOutputRegister(digitalPinToPort(10))=digitalPinToBitMask(10);} // D10 = PB4   PB2
#define open_line3 {*portOutputRegister(digitalPinToPort(11))=digitalPinToBitMask(11);} // D11 = PB5   PB3
#define open_line4 {*portOutputRegister(digitalPinToPort(12))=digitalPinToBitMask(12);} // D12 = PB6   PB4
#define open_line5 {*portOutputRegister(digitalPinToPort(13))=digitalPinToBitMask(13);} // D13 = PB7   PB5
#define open_line6 {*portOutputRegister(digitalPinToPort(3))=digitalPinToBitMask(3);}   // D3  = PE5   PD3
#define open_line7 {*portOutputRegister(digitalPinToPort(4))=digitalPinToBitMask(4);}   // D4  = PG5   PD4
...
public:
  ...
  void _IO_Init() {
#if defined(__AVR_ATmega1280__) || defined(__AVR_ATmega2560__)
    DDRF = 0xff; // set all pins direction to output of PortF
    DDRH = 0xff; // set all pins direction to output of PortH
    DDRB = 0xff; // set all pins direction to output of PortB
    DDRE = 0xff; // set all pins direction to output of PortE
    DDRG = 0xff; // set all pins direction to output of PortG

    PORTF = 0x00; // set all pins output is low of PortF
    PORTH = 0x00; // set all pins output is low of PortH
    PORTB = 0x00; // set all pins output is low of PortB
    PORTE = 0x00; // set all pins output is low of PortE
    PORTG = 0x00; // set all pins output is low of PortG
#elif defined(__AVR_ATmega328__) || defined(__AVR_ATmega168__)
    DDRD = 0xff; // set all pins direction to output of PortD
    DDRC = 0xff; // set all pins direction to output of PortC
    DDRB = 0xff; // set all pins direction to output of PortB
  ...
  void _TC2_Init() {
    ...
    // Timer2/Timer4 (8bit) Settings:
    // prescaler (frequency divider) values:   CSx2    CSx1   CSx0
    //                                           0       0      0    stopped
    //                                           0       0      1      /1
    //                                           0       1      0      /8
    //                                           0       1      1      /32
    ...
    //                                           1       1      0      /256
    //                                           1       1      1      /1024
    // TCNT2/TCNT4 increments every  = 16MHz / prescaler
    ...
    TCNT2   = 0xff;
#elif defined(__AVR_ATmega32U4__)
    TCCR4B |= ((1<<CS42)|(1<<CS41)|(1<<CS40)); // prescaler 128
    TCCR4C &=~ (1<<PWM4D);
